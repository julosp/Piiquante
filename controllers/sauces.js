...
exports.createSauces = (req, res) => {
  ...
  const sauce = new sauces({
    //crée une sauces via le model sauce
    ...sauceObject,
...
exports.modifySauces = (req, res, next) => {
  ...
  const sauceObject = req.file //recuperation de la sauce
    ? {
        ...JSON.parse(req.body.sauce), //recup des inputs
        imageUrl: `${req.protocol}://${req.get("host")}/images/${
          //recup de l'image
          req.file.filename
        }`,
  ...
  sauces
    ...
    .then((sauce) => {
      if (sauce.userId != req.auth.userId) {
        //si le l'user id de la sauce != a l'id de l'user
        ...
        sauces
          .updateOne(
            //sinon on update la sauce selon les inputs
            { _id: req.params.id },
            { ...sauceObject }
          )
